...
These elements are described in different folders. All the modelling happens within the `openfisca_dubai` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-dubai.git
cd openfisca-dubai
...
Substitute your package's country name for `openfisca_dubai` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_dubai/situation_examples/couple.json \
